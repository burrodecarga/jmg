                ...
                <table id="sede" class="table text-sm table-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('department') . '/' . __('municipality') }}</th>
                            <th>{{ __('coordinators') }}</th>
                    ...
                    <tbody class="text-sm">
                        @foreach ($sedes as $sede)
                            <tr class="odd:bg-slate-100">
                                <td width="25%" class="text-xs text-left">{{ $sede->school }}<br>
                                    {{ $sede->name }}</td>
                                <td width="15%" class="text-left">{{ $sede->department }}<br>
                                    {{ $sede->municipality }}</td>
                                <td>
                                    @foreach ($sede->coordinadores as $coordinador)
                                        <p class="m-0">{{ $coordinador->name }}</p>
                                    @endforeach
                                </td>
                                <td class="grid grid-cols-2 gap-3 text-center" width="">
